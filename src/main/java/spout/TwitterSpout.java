...
public class TwitterSpout extends BaseRichSpout {
    ...
    public TwitterSpout(String consumerKey, String consumerSecret, String accessToken, String accessTokenSecret) {
        if (consumerKey == null || consumerSecret == null || accessToken == null || accessTokenSecret == null) { throw new RuntimeException("Twitter4j OAuth field cannot be null"); }
    ...
    public void open(Map map, TopologyContext topologyContext, SpoutOutputCollector spoutOutputCollector) {
        ...
        _configurationBuilder.setOAuthConsumerKey(_consumerKey).setOAuthConsumerSecret(_consumerSecret).setOAuthAccessToken(_accessToken).setOAuthAccessTokenSecret(_accessTokenSecret);
        ...
        _twitterStream.addListener(new StatusListener() {
            ...
            public void onStatus(Status status) {
                if (meetsConditions(status))
                    _msgs.offer(status.getUser().getScreenName());
            }
            @Override
